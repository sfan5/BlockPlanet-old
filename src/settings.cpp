...
void Settings::set(std::string name, const char *value)
{
	JMutexAutoLock lock(m_mutex);
...
void Settings::setDefault(std::string name, std::string value)
{
	JMutexAutoLock lock(m_mutex);
...
bool Settings::getFlag(std::string name)
{
	try
	{
		return getBool(name);
...
// Asks if empty
bool Settings::getBoolAsk(std::string name, std::string question, bool def)
{
...
float Settings::getFloat(std::string name)
{
	return stof(get(name));
}

u16 Settings::getU16(std::string name)
{
	return stoi(get(name), 0, 65535);
}

u16 Settings::getU16Ask(std::string name, std::string question, u16 def)
{
	// If it is in settings
	if(exists(name))
	{
...
s16 Settings::getS16(std::string name)
{
	return stoi(get(name), -32768, 32767);
}

s32 Settings::getS32(std::string name)
{
	return stoi(get(name));
}

v3f Settings::getV3F(std::string name)
{
	v3f value;
	Strfnd f(get(name));
	f.next("(");
...
v2f Settings::getV2F(std::string name)
{
	v2f value;
	Strfnd f(get(name));
	f.next("(");
...
u64 Settings::getU64(std::string name)
{
	u64 value = 0;
...
void Settings::setS32(std::string name, s32 value)
{
	set(name, itos(value));
}

void Settings::setFloat(std::string name, float value)
{
	set(name, ftos(value));
}

void Settings::setV3F(std::string name, v3f value)
{
...
void Settings::setV2F(std::string name, v2f value)
{
	std::ostringstream os;
	os<<"("<<value.X<<","<<value.Y<<")";
	set(name, os.str());
...
void Settings::setU64(std::string name, u64 value)
{
	std::ostringstream os;
	os<<value;
	set(name, os.str());
...
void Settings::updateValue(Settings &other, const std::string &name)
{
	JMutexAutoLock lock(m_mutex);
...
void Settings::update(Settings &other)
{
	JMutexAutoLock lock(m_mutex);
